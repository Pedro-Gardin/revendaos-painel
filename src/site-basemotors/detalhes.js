...
async function iniciar() {
  ...
  document.title = `${c.marca} ${c.modelo} | Base Motors`;
  ...
  document.querySelector('#description').textContent = c.desc ||
    `${c.marca} ${c.modelo} selecionado pela Base Motors, com procedência, revisão e documentação em dia. Consulte nossa equipe para conhecer todos os detalhes.`;
  document.querySelector('#whatsapp').href =
    `https://wa.me/5554981461712?text=${encodeURIComponent(`Olá! Vi o site e tenho interesse no ${c.marca} ${c.modelo} (${c.preco}). Ainda disponível?`)}`;
